...
public class CustomerServiceImpl implements ICustomerService {
	...
	public List<Customer> findListHasAssociation(String decidedzoneId) {
		...
		System.out.println("findListHasAssociation======================list======="+list);
	...
	public String findDecidedzoneIdByAddress(String address) {
	  ...
	  String decidedzoneId = jdbcTemplate.queryForObject(sql, String.class, address);
	   return decidedzoneId;
		...
		/*String sql = "select * from t_customer where address = ?";
		List<Customer> list = jdbcTemplate.query(sql, new RowMapper<Customer>(){
			public Customer mapRow(ResultSet rs, int arg1) throws SQLException {
				int id = rs.getInt("id");//根据字段名称从结果集中获取对应的值
				String name = rs.getString("name");
	...
	System.out.println("list========================================"+list);
		for(Customer customer:list){
			String decidedzoneId = customer.getDecidedzone_id();
			System.out.println("decidedzoneId============================"+decidedzoneId);
			return decidedzoneId;
		}
		System.out.println("secfwfew======================================================");
		return null;*/
